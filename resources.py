...
class wordleClass():
    def __init__(self, wordSize: int, wordList: tuple):
    ...
    def winners(self):
        return sorted(self.hallOfFame.items(), key=lambda x:x[1], reverse=True)
    ...
    def attempt(self, guess, username):
        ...
        if sum(self.status) == 10 * self.wordsize:        # if missed everyword
            self.response += "\nburro"
